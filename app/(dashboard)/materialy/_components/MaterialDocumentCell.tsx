...
export function MaterialDocumentCell({ file, dateLabel }: Props) {
  ...
  const showDate = dateLabel && dateLabel !== "—";

  if (!file) {
    return (
      <div className="text-gray-600">
        {showDate ? <div>{dateLabel}</div_END> : <span className="text-gray-400">{"\u2014"}</span>}
      </div_END>
    );
  }
  ...
  return (
    <div className="min-w-[8rem] max-w-[14rem] space-y-1">
      {showDate ? <div className="text-gray-600">{dateLabel}</div_END> : null}
