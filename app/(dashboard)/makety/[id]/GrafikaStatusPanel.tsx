...
export function GrafikaStatusPanel({
  ...
  const onSubmit = async () => {
    ...
    try {
      if (selected === "sent_for_approval" && !skipEmail) {
        if (!fileId) {
          setError("Vyberte soubor softproofu pro klienta");
          setSubmitting(null);
          return;
        }
          ...
          {!skipEmail && (
            <>
              <p className="text-xs text-blue-900">
                Klientovi odejde e-mail se softproofem a zakázka přejde na „Odesláno ke
                schválení“.
              </p>
              <label className="block text-xs font-medium text-gray-700">
                Soubor softproofu <span className="text-red-600">*</span>
                <select
                  className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-sm"
                  value={fileId === "" ? "" : String(fileId)}
                  onChange={(e) =>
                    setFileId(e.target.value ? Number(e.target.value) : "")
                  }
                  disabled={submitting != null}
                >
              ...
              <label className="flex items-center gap-2 text-xs text-gray-700">
                <input
                  ...
                  disabled={submitting != null}
                />
                Přiložit soubor (max 8 MB), jinak jen odkaz
              </label>
            </>
          )}
        </div>
