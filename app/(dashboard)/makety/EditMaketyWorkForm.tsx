...
import { GrafikaImlFields, type GrafikaImlInitial } from "./GrafikaImlFields";
...
export function EditMaketyWorkForm({
  maketaId,
  workType,
  assigneeUsers,
  initial,
  ...
  const imlInitial: GrafikaImlInitial = {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isGrafika) {
      payload.data_kind = String(fd.get("data_kind") ?? "nova_data");
      ...
      payload.final_approver_user_id =
        String(fd.get("final_approver_user_id") ?? "").trim() || null;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-4xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {isGrafika && (
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <select
              name="data_kind"
              defaultValue={initial.data_kind ?? "nova_data"}
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
            >
      ...
      {isGrafika && (
        <GrafikaWorkflowPicker
          ...
          initial={{
            ...
            prepress_user_id: initial.prepress_user_id ?? null,
            final_approver_user_id: initial.final_approver_user_id ?? null,
          }}
          includeAssignee
        />
      )}

      {isGrafika && <GrafikaImlFields initial={imlInitial} />}
      ...
      <p className="text-sm text-gray-500">Přílohy upravíte na detailu zakázky po uložení.</p>
