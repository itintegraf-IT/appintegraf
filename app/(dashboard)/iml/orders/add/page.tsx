...
export default function ImlOrderAddPage() {
  ...
  const addSelectedItem = () => {
    ...
    setError("");
    setItemAddedMessage("Položka přidána. Můžete přidat další produkt nebo vytvořit objednávku.");
  ...
  const submitOrder = async (withSupervisorOverride: boolean): Promise<boolean> => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const ok = await submitOrder(supervisorAck);
  ...
  const confirmSupervisorModal = async () => {
    ...
    setSaveMessage("");
    try {
      const ok = await submitOrder(true);
      if (!ok) setLoading(false);
    } catch {
      setError("Chyba při ukládání");
      setLoading(false);
