...
import {
  autoMapDieCutHeaders,
  DIE_CUT_IMPORT_FIELDS,
  type DieCutColumnMapping,
} from "@/lib/iml/die-cuts-import";

type Mapping = DieCutColumnMapping;
...
export default function ImlDieCutsImportPage() {
  ...
  const applyParsedMatrix = useCallback((matrix: string[][]) => {
    if (matrix.length > 0) {
      const hdrs = matrix[0].map((h, i) => (h ? String(h) : `Sloupec ${i + 1}`));
      setHeaders(hdrs);
      setRows(matrix.slice(1, 21));
      setMapping(autoMapDieCutHeaders(hdrs));
    } else {
      setHeaders([]);
      setRows([]);
      setMapping({});
      ...
      if (ext.endsWith(".xlsx") || ext.endsWith(".xls")) {
        ...
        reader.onload = (e) => {
          try {
            ...
            applyParsedMatrix(arr as string[][]);
          } catch {
        ...
        reader.onload = () => {
          ...
          applyParsedMatrix(parseCsv(text));
        };
  ...
  const canImport = () => {
    return file && typeof mapping.label_shape_code === "number" && mapping.label_shape_code >= 0;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/iml/die-cuts/import", { method: "POST", body: formData });
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Import výseků z CSV/Excel</h1>
          <p className="mt-1 text-gray-600">
            Nahrajte soubor, zobrazte náhled a přetáhněte sloupce na cílová pole. Existující kódy
            tvaru se aktualizují.
          </p>
        </div>
        <Link
          href="/iml/die-cuts"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {result && (
          <div className="rounded-lg bg-green-50 p-3 text-sm text-green-800">
            Importováno: {result.imported} výseků ({result.created} nových, {result.updated}{" "}
            aktualizovaných).
          ...
          <>
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <p className="mb-4 text-xs text-gray-500">
                Po nahrání souboru se mapování pokusí doplnit automaticky podle názvů sloupců.
              </p>
