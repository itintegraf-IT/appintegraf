...
export function ResourceApproveRejectButtons({ reservationId, title }: Props) {
  ...
  const handleApprove = async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/calendar/reservations/${reservationId}/approve`, {
        method: "POST",
      ...
      {rejectModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-lg">
            <h3 className="text-lg font-semibold text-gray-900">Zamítnout rezervaci</h3>
            <p className="mt-2 text-sm text-gray-600">
              Rezervace „{title}“ bude zamítnuta. Důvod bude odeslán žadateli.
            </p>
