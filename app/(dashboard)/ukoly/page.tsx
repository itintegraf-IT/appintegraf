...
export default async function UkolyListPage({
  searchParams,
}: {
  searchParams: Promise<{ status?: string; dept?: string; term?: string; created?: string; completed?: string }>;
}) {
  ...
  return (
    <div className="space-y-5">
      ...
      {completed && (
        <div className="rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
          Splnění úkolu bylo potvrzeno. Úkol je přesunut do archivu.
        </div>
      )}
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
            >
              <option value="">Všechna moje oddělení</option>
              {myDepartments.map((d) => (
          ...
          <div className="flex items-end gap-2">
            ...
            <Link href="/ukoly" className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
              Reset
            </Link>
          </div>
        </div>
