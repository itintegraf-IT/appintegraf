...
export function ListGroupHeader({ list }: { list: ListData }) {
  ...
  return (
    <header
      ref={setNodeRef}
      className={`sticky top-0 z-10 flex items-center gap-2 border-b border-[hsl(var(--notion-fg)/0.09)] px-4 py-2 backdrop-blur transition-colors ${
        isOver
          ? "bg-[hsl(var(--info)/0.08)]"
          : "bg-[hsl(var(--notion-canvas))]/95"
      }`}
    >
      <span
        ...
        aria-hidden
      />
      <h3 className="text-xs font-semibold uppercase tracking-wider text-[hsl(var(--notion-fg))]">
        {list.name}
      </h3>
      <span className="ml-auto text-xs tabular-nums text-[hsl(var(--notion-fg)/0.5)]">
        {totalCount}
