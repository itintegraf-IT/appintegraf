 ...
 * Landscape: jméno vlevo nahoře, logo vpravo nahoře, uprostřed prázdno,
 * dole 3 řádky (firma/adresa | M/E/W) s vodorovnými linkami.
 */
...
/** Linky jako na tisku — světlejší šedá. */
...
export function buildOutlookContactSignatureHtml(data: ContactSignatureInput, assetBaseUrl: string): string {
  ...
  // Buňka spodní mřížky — linka nahoře u 1. řádku, pod každým řádkem; mezera mezi sloupci.
  const leftCell = (content: string, extraBorder: string, bold = false) =>
    `<td style="vertical-align:middle;width:48%;padding:1.5px 2.5mm 1.5px 0;border-bottom:${RULE};${extraBorder}${bold ? "font-weight:bold;" : ""}">${content}</td>`;
  const rightCell = (content: string, extraBorder: string) =>
    `<td style="vertical-align:middle;width:52%;padding:1.5px 0 1.5px 2.5mm;border-bottom:${RULE};${extraBorder}">${content}</td>`;

  const topBorder = `border-top:${RULE};`;

  const bottomGrid = `<table cellpadding="0" cellspacing="0" border="0" role="presentation" width="100%" style="width:100%;border-collapse:collapse;${FONT}">
<tr>
...
${rightCell(webInner, "")}
  ...
  return `<table cellpadding="0" cellspacing="0" border="0" role="presentation" width="${CARD_WIDTH_MM}" height="${CARD_HEIGHT_MM}" style="width:${cardW};height:${cardH};max-width:${cardW};table-layout:fixed;border-collapse:collapse;background:#ffffff;${FONT}">
<tr>
...
<img src="${escapeHtml(logoSrc)}" width="52" height="52" alt="Integraf" style="display:block;width:14mm;height:14mm;border:0;margin:0 0 0 auto;" />
</td>
</tr>
</table>
</td>
