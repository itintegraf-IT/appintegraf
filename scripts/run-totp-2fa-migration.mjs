/**
 * Spustí prisma/migrations/20260518_totp_2fa.sql proti databázi z .env (DATABASE_URL).
 * Použití: node scripts/run-totp-2fa-migration.mjs
 */
...
async function main() {
  const sqlPath = join(root, "prisma", "migrations", "20260518_totp_2fa.sql");
  ...
  const enrollmentPath = join(root, "prisma", "migrations", "20260519_totp_enrollment_required.sql");
  if (existsSync(enrollmentPath)) {
    let enrollSql = readFileSync(enrollmentPath, "utf8");
    enrollSql = enrollSql
      .split("\n")
      .filter((line) => !/^\s*--/.test(line))
      .join("\n");
    const enrollStmts = enrollSql
      .split(/;\s*\n/)
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
    statements.push(...enrollStmts);
  }
