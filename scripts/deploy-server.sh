...
makety_quote_exit=$?
npm run db:makety-iml-applied
makety_iml_exit=$?
set -e
if [[ "$makety_migrate_exit" -ne 0 || "$makety_work_type_exit" -ne 0 || "$makety_queue_exit" -ne 0 || "$makety_quote_exit" -ne 0 || "$makety_iml_exit" -ne 0 ]]; then
  echo "Upozornění: některá makety migrace skončila chybou (migrate=$makety_migrate_exit work_type=$makety_work_type_exit queue=$makety_queue_exit quote=$makety_quote_exit iml=$makety_iml_exit)."
  ...
  echo "    npm run db:makety-iml-applied"
  echo ""
fi
...
if [[ "${#SQL_FILES[@]}" -gt 0 ]]; then
  ...
  for f in "${SQL_FILES[@]}"; do
    ...
    elif [[ -f "prisma/manual-migrations/$f" ]]; then
      sql_path="prisma/manual-migrations/$f"
    else
