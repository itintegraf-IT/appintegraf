/**
 * Migrace modulu Požadavky (equipment_requester_user_id) + Helpdesk tabulky.
 * Použití: npm run db:pozadavky-migrate
 */
...
const SQL_FILES = [
  join(root, "prisma", "migrations", "20260623120000_equipment_requester_user_id", "migration.sql"),
  join(root, "prisma", "migrations", "20260623130000_helpdesk", "migration.sql"),
];
...
async function main() {
  ...
  await conn.end();
  console.log("\nMigrace Požadavky + Helpdesk dokončena.");
}
