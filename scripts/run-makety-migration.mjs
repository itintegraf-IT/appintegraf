/**
 * Vytvoří tabulky modulu Makety (CREATE TABLE IF NOT EXISTS).
 * Použití: npm run db:makety-migrate
 */
...
const SQL_CANDIDATES = [
  join(root, "prisma", "migrations", "20260601000000_makety_module", "migration.sql"),
  join(root, "prisma", "migrations", "20260601_makety_module.sql"),
];
...
async function main() {
  const sqlPath = SQL_CANDIDATES.find((p) => existsSync(p));
  if (!sqlPath) {
    console.error("Nenalezen SQL soubor migrace Makety.");
    process.exit(1);
  }
  ...
  await conn.end();
  console.log("\nMigrace modulu Makety dokončena.");
}
